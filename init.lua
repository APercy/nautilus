...
core.register_lbm({                            -- this is to remove old bright water nodes after server crash etc
...
if(core.get_translator ~= nil) then
    nautilus.S = core.get_translator(core.get_current_modname())
else
...
nautilus.is_minetest = core.get_modpath("player_api")
nautilus.is_mcl = core.get_modpath("mcl_player")

nautilus.gravity = tonumber(core.settings:get("movement_gravity")) or 9.8
...
nautilus.have_air = core.settings:get_bool("nautilus_air", nautilus.have_air)
...
function nautilus.clone_node(node_name)
    if not node_name and not type(node_name) == 'string' then
        return
    end
...
dofile(core.get_modpath("nautilus") .. DIR_DELIM .. "nautilus_control.lua")
dofile(core.get_modpath("nautilus") .. DIR_DELIM .. "nautilus_fuel_management.lua")
dofile(core.get_modpath("nautilus") .. DIR_DELIM .. "nautilus_air_management.lua")
dofile(core.get_modpath("nautilus") .. DIR_DELIM .. "nautilus_custom_physics.lua")
...
function nautilus.stand(player)
        ...
        if item_def.overload_drop then
            for _,item in pairs(item_def.overload_drop) do
                if player then
                    ...
                    if item_to_add then
                        core.add_item(player:getpos(), item_to_add)
                    else
                        core.add_item({x=pos.x+math.random()-0.5,y=pos.y,z=pos.z+math.random()-0.5},item)
                    end
                else
                    core.add_item({x=pos.x+math.random()-0.5,y=pos.y,z=pos.z+math.random()-0.5},item)
                end
    ...
    if node then
        ...
        if (node_def.liquidtype=="none") and (node_def.drowning==0) then
            if (self.air < nautilus.REAIR_ON_AIR) then
                ...
                core.chat_send_player(player:get_player_name(), S("Nautilus has been filled with fresh air."))
            end
        else
...
-- entity
--
core.register_entity("nautilus:boat", {
    initial_properties = {
        physical = true,
    ...
    on_activate = function(self, staticdata, dtime_s)
        ...
        local pointer=core.add_entity(pos,'nautilus:pointer')
        ...
        if nautilus.have_air then
            local pointer_air=core.add_entity(pos,'nautilus:pointer_air')
    ...
    on_step = function(self, dtime)
        ...
        if self.deep_limit then
            ...
            if (deep>self.deep_limit) then
                ...
                if (self.deep_time > 1) then
                    ...
                    --core.sound_play("nautilus_crushing", {
                    core.sound_play("collision", {
                    ...
                    if (self.hull_integrity~=nil) then
                        ...
                        if (self.hull_integrity <= 0) then
                            --core.sound_play("nautilus_hull_break", {
        ...
        if self.energy > 0 then
            ...
            if self.pointer:get_luaentity() then
                ...
                self.pointer=core.add_entity(nautilus.GAUGE_FUEL_POSITION,'nautilus:pointer')
                self.pointer:set_attach(self.object,'',nautilus.GAUGE_FUEL_POSITION,{x=0,y=0,z=energy_indicator_angle})
            end
        end
        if self.energy <= 0 then
        ...
        if nautilus.have_air and is_attached then
            if (self.air > 0) then
                ...
                if self.pointer_air:get_luaentity() then
                    ...
                    --in case it have lost the entity by some conflict
                    self.pointer_air=core.add_entity(nautilus.GAUGE_AIR_POSITION,'nautilus:pointer_air')
                    self.pointer_air:set_attach(self.object,'',nautilus.GAUGE_AIR_POSITION,
                            {x=0,y=0,z=air_indicator_angle})
                end
...
function nautilus.put_light(object, name)
    ...
    if pos then
        ...
        if n and n.name == 'default:water_source' then
            core.set_node(pos, {name='nautilus:water_light'})
            --local timer = core.get_node_timer(pos)
            --timer:set(10, 0)
            core.after(10,function(pos)
                local node = core.get_node_or_nil(pos)
...
local nautilus_newnode = nil
if nautilus.is_mcl then
    nautilus_newnode = nautilus.clone_node('mcl_core:ice')
else
    nautilus_newnode = nautilus.clone_node('default:ice')
end
if nautilus_newnode then
    ...
    core.register_node('nautilus:water_light', nautilus_newnode)
end
...
nautilus.on_place = function(itemstack, placer, pointed_thing)
    ...
    if nodedef.liquidtype ~= "none" then
        ...
        node_below = core.get_node(pointed_pos).name
        nodedef = core.registered_nodes[node_below]
        if nodedef.liquidtype == "none" then
            core.chat_send_player(placer:get_player_name(), S("Nautilus have to be placed on deeper water."))
            return
        end
        ...
        if (nodedef.liquidtype ~= "none") or (nodedef.buildable_to==false) then
            core.chat_send_player(placer:get_player_name(), S("Nautilus have to be placed on open water surface"))
            return
        end
        ...
        local boat = core.add_entity(pointed_pos, "nautilus:boat", staticdata)
...
if core.settings:get_bool("nautilus_variants", true) then
    dofile(core.get_modpath("nautilus") .. DIR_DELIM .. "nautilus_variants.lua")
end
